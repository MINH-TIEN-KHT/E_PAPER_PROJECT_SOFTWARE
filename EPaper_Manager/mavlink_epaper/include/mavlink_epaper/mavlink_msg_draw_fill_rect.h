...
#define MAVLINK_MSG_ID_DRAW_FILL_RECT_LEN 8
...
#define MAVLINK_MESSAGE_INFO_DRAW_FILL_RECT { \
	"DRAW_FILL_RECT", \
	4, \
	{  { "xpos", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_draw_fill_rect_t, xpos) }, \
         { "ypos", NULL, MAVLINK_TYPE_UINT16_T, 0, 2, offsetof(mavlink_draw_fill_rect_t, ypos) }, \
         { "height", NULL, MAVLINK_TYPE_UINT16_T, 0, 4, offsetof(mavlink_draw_fill_rect_t, height) }, \
         { "width", NULL, MAVLINK_TYPE_UINT16_T, 0, 6, offsetof(mavlink_draw_fill_rect_t, width) }, \
         } \
}
						       ...
						       uint16_t xpos, uint16_t ypos, uint16_t height, uint16_t width)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, 8, 236);
}
						           ...
						           uint16_t xpos,uint16_t ypos,uint16_t height,uint16_t width)
{
	...
	msg->msgid = MAVLINK_MSG_ID_DRAW_FILL_RECT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 8, 236);
}
...
{
	return mavlink_msg_draw_fill_rect_pack(system_id, component_id, msg, draw_fill_rect->xpos, draw_fill_rect->ypos, draw_fill_rect->height, draw_fill_rect->width);
}
...
static inline void mavlink_msg_draw_fill_rect_send(mavlink_channel_t chan, uint16_t xpos, uint16_t ypos, uint16_t height, uint16_t width)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[8];
	_mav_put_uint16_t(buf, 0, xpos);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DRAW_FILL_RECT, buf, 8, 236);
#else
	mavlink_draw_fill_rect_t packet;
	packet.xpos = xpos;
	packet.ypos = ypos;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_DRAW_FILL_RECT, (const char *)&packet, 8, 236);
#endif
}
...
#else
	memcpy(draw_fill_rect, _MAV_PAYLOAD(msg), 8);
#endif
}
